...
#us pop numbers from https://www.census.gov/library/visualizations/interactive/population-increase-2018.html 
...
#note: here, regions_weighted_allyears is yearly visits
...
#note: subregions_weighted_allyears here is yearly average visits
...
```{r Figure 1}
#note: regions_weighted_allyears here is REDEFINED and now is monthly average visits
...
#note: subregions_weighted_allyears is redefined and is now monthly average visits
...
#repeat this plot with log model phases 
state_sinusoids_parameters_tojoin_log <- state_sinusoids_parameters_log |> mutate(Phase_corrected = Phase %% 12, 
                                                                          id = State) |> select(id, Phase_corrected)
continental_phases_log <- left_join(continental_fortified_tojoin, state_sinusoids_parameters_tojoin_log) 
continental_phases_noSC_log <- continental_phases_log
continental_phases_noSC_log[which(continental_phases_noSC_log$id == "South Carolina"),8] = NA

#becomes supplementary figure 24
state_phase_map_noSC_log <- ggplot() + 
  geom_polygon(data = continental_phases_noSC_log, aes(x = long, y = lat, group = group, fill = Phase_corrected), color="black") +
  geom_polygon_pattern(data = sc, aes(x = long, y = lat, group= group), fill = "gray", pattern = 'stripe', 
                       pattern_spacing = 0.005, pattern_alpha = 0.5) +
  theme_void() + ggtitle("") + scale_fill_gradient(low = "#30239B", high =  "#FFFFFF", breaks = c(0, 1, 2, 3), 
                                                   labels = c("Dec", "Jan", "Feb", "Mar"),
...
state_sinusoids_subregions_log <- ggplot() + 
  geom_point(aes(x = MONTH, y = log_mean_weighted_cases_per_thousand, color = Region), data = state_sinusoids_withCIs_cont_sub_log, size = 0.5) + 
  geom_ribbon(aes(x = MONTH, ymin = lower, ymax = upper, fill = Region), data = state_sinusoids_withCIs_cont_sub_log, alpha = 0.2) +
  geom_line(aes(x = MONTH, y = prediction, color = Region), data = state_sinusoids_withCIs_cont_sub_log, linetype = 2) +
  scale_color_manual(values= subcolors_df$region_color, labels=gsub("_", " ",subcolors_df$Region , fixed=TRUE)) +
  scale_fill_manual(values = subcolors_df$region_color, labels=gsub("_", " ",subcolors_df$Region , fixed=TRUE)) +
  facet_wrap(~STATE) +
  # ggtitle("Streptococcal Pharyngitis Visits State Sinusoidal Fits") +
...
Fit sinusoids to region trends and plot. Becomes Supplementary Figure 12. 
...
Plot regional sinusoid phases. Becomes Supplementary Figure 12.
...
Fit sinusoids to subregional data and plot. Becomes Supplementary Figure 11.
```{r supplementary figure 11a}
...
Plot subregion phases. Becomes Supplementary Figure 11b.
```{r subregion phases, supplementary figure 11b}
...
Calculate subregion centroids. Becomes Supplementary Figure 13. 
...
Load in trimmed school start data, calculate uptick points and plot. Becomes Supplementary Figure 15 and Figure 3.
```{r school start date,fig 3, figS14}
#summarize school start data
start_date_summary <- read_csv("../output/school_starts_summary.csv")
...
cor(as.numeric(df_toplot$Date_SchoolStart), as.numeric(df_toplot$`Date_Min Visit Date`)) #raw correlation 0.88 (use bootstrapped)
...
make acf plots, which are in base R plotting
```{r}
